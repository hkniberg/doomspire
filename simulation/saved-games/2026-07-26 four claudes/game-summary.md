...
[![Final board](board.png)](board.png)
...
- **All four players misread the adventure and oasis tiles, and it was the game state's fault.** The stringified board never said how many adventure tokens a tile held: an adventure tile printed as `Tier 1 adventure tile (explored)` whether it had two cards or none, and an oasis printed as a bare `Tier 1 oasis` with nothing to suggest it was an adventure tile at all. The models filled the gap by guessing, and they all guessed "empty" — Oliver and Fabian both wrote reasoning about walking "through the empty oasis," and Sarah twice stepped onto a stocked adventure tile believing it was inert and drew a card that wrecked her turn. The net effect on this match: all four oases sat untouched with two cards each for 14 rounds, and the round-5 Oasis Bloom fate card restocked nothing because nothing had been depleted. Treat any conclusion about adventure-tile pacing from this game as unreliable. Fixed afterwards — token counts are now reported for both tile types, and the rules now say a card is drawn when a knight *ends its movement on* the tile rather than "enters" it.
